...
K (≤10^5), the total number of problems, 
and MM (≤10^5), the total number of submittions. 
It is then assumed that the user id's are 5-digit numbers from 00001 to N, and the problem id's are from 1 to K. 
The next line contains K positive integers p[i] (i=1, ..., KK), where p[i] corresponds to the full mark of the i-th problem. 
Then MM lines follow, each gives the information of a submittion in the following format:
